...
namespace Chau.Controllers
{
    public class DMKhachHangController : Controller
    {
        ...
        {
            var lisofdata = _context.DMKhachHang.ToList();
        ...
        public ActionResult Create(Models.DMKhachHang model) // yeu cau POST tu form tao moi va them mot bang ghi moi vao NhomHangs
        {
            // Chuyển đổi định dạng ngày từ MM/dd/yyyy sang dd/MM/yyyy


            _context.DMKhachHang.Add(model); // nhan tham so( model) la doi tuong NhomHang chua thong tin nguoi dung nhap vao form va them vao NhomHangs
        ...
        public ActionResult Delete(string id)
        {
            ...
            var item = _context.DMKhachHang.Find(id);
        ...
        public ActionResult DeleteConfirmed(string id)
        {
            ...
            var model = _context.DMKhachHang.Find(id.ToString());
            if (model == null)
            {
                return HttpNotFound();
            }
            // Xóa đối tượng
            _context.DMKhachHang.Remove(model);
